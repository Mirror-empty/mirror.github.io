...
确保一个类职业一个实例，并提供该实例的全局访问点。

### Class Diagram

使用一个私有静态变量以及一个公有静态函数来实现。

单例模式有一个特点就是不允许外部直接创建，也就是``new Singleton()``，因此这里在默认的构造函数上添加了私有属性 private。

私有构造函数保证了不能通过构造函数来创建对象实例，只能通过公有静态函数返回唯一的私有静态变量。
![zbxdGn.png](https://s1.ax1x.com/2022/12/18/zbxdGn.png)
